...
plot(r,N_ct);
xlabel("Round time");
ylabel("Comprehensive trust values of non-malicious nodes");
title("Comprehensive trust for non-malicious nodes(N_ct) vs Round time(r)");
...
mn_w = 1:1:mnwa;
plot(mn_w,r)
xlabel("Number of malicious nodes under warmhole attack")
ylabel("Round time")
title("Speed with which the malicious nodes are recognised");
...
%     disp(width(coordinates));
    ...
    if C_T(c_t) > 0
        ...
        scatter3(coordinates(:,1),coordinates(:,2),coordinates(:,3),100,idx,'filled')
    end
...
%     disp(width(cdt));
    ...
    if 0 < C_T(ctx) < 1.25
        [idx,ch] = kmeans(cdt,K);
        scatter3(cdt(:,1),cdt(:,2),cdt(:,3),50,idx,'filled');
        hold on;
    end
    if C_T(ctx) >= 1.25
        plot3(S(ctx).xd,S(ctx).yd,S(ctx).zd,'p','MarkerSize',16,'MarkerFaceColor','k');
    end
    title("Clustering of the WSN")
    xlabel("Length of WSN")
    ylabel("Breadth of WSN")
...
%     disp(width(cdt));
    ...
    ch = [ct];
    if C_T(cty) > 1.25
        [idx,ch,D] = kmeans(c_dt,K);
        scatter3(c_dt(:,1),c_dt(:,2),c_dt(:,3),50,idx,'filled');
        hold on;
    end
