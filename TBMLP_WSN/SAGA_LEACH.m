...
figure, plot(1:no_of_gen, res_fin, 'b:', 'linewidth', 2); 
xlabel ('Time Steps (Rounds)'); 
ylabel('Dead Nodes'); 
grid on; 
legend ('LEACH-SAGA Protocol'); 
title('THE NUMBER OF DEAD NODES');

figure, plot(1:no_of_gen, res_fin2 (1:no_of_gen), 'y:', 'linewidth', 2); 
xlabel ('Time Steps (Rounds)'); 
ylabel('Total Packet Received'); 
grid on; 
legend ('LEACH-SAGA Protocol'); 
title('BASE STATION RECEIVES DATA PACKETS');

figure, plot (1:no_of_gen, res_fin3(1:no_of_gen),'c:', 'linewidth',2);
xlabel ('Time Steps (Rounds)'); 
